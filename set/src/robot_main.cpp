...
Service() {
    ...
    pubMapChange = nh.advertise<std_msgs::String>("/map_change", 1);
    ...
    sub_module_status = nh.subscribe("/state", 1, &Service::CheckModule, this);
};
...
void CheckModule(const std_msgs::String module) {
    ...
    if (strcmp(module.data.c_str(), "serving") == 0) {
        m_serve = true;
        _SERVE_FLAG = S_WAIT;
...
void CheckFirebase(const std_msgs::String firebase) {
    ...
    if (m_hotel && _HOTEL_FLAG == H_WAIT) {
        ...
        if (strcmp(firebase.data.c_str(), "Home_Success") == 0) {
            ROS_INFO("Return.");
            ultra.data = "180";
...
void CheckUltra(const std_msgs::String ultra)
{
    if (m_serve) {
        if (strcmp(ultra.data.c_str(), "end") == 0) {
            ...
            if (_SERVE_FLAG == S_RETRIEVAL_ARR) {
...
void CheckElevator1(const std_msgs::String elevator1)
{
    if (m_hotel){
        ...
        if (strcmp(elevator1.data.c_str(), "Second_Open") == 0) {
...
void CheckElevator2(const std_msgs::String elevator2)
{
    if (m_hotel) {
        ...
        if (strcmp(elevator2.data.c_str(), "Second_Open") == 0) {
...
std_msgs::String fb, ultra, direction, ev, hotel, map;
