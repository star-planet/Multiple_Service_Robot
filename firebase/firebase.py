...
class Listener:
    def __init__(self):
        ...
        self.cred = credentials.Certificate('key.json') # Key Certification

        firebase_admin.initialize_app(self.cred, {
            'databaseURL': 'https://project-6dfc7-default-rtdb.firebaseio.com/' # Database url
        })
        rospy.loginfo("[Firebase ON]")
        self.talker()

    #아두이노 혹은 라즈베리파이가 Firebase의 데이터를 업데이트시켜주는 코드
